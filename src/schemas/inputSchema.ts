...
/**
 * Validates and normalizes input variables based on VALIDATION_RULES
 */
export const validateInputs = (input: InputVariables): InputVariables => {
  const validated = { ...input };

  // Normalization
  if (validated.role) validated.role = validated.role.trim();
  if (validated.task_description) validated.task_description = validated.task_description.trim();

  // Tier-Gated Enforcement
  if (validated.complexity_tier === "free") {
    validated.custom_instructions = "";
    validated.multi_step_enabled = false;
    validated.chain_of_thought = false;
  } else if (validated.complexity_tier === "pro") {
    validated.chain_of_thought = false;
  }
